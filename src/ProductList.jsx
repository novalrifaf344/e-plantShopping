...
function ProductList({ onHomeClick }) {
  ...
  const cartCount = cartItems.reduce((t, it) => t + (it.quantity || 0), 0);
  const isInCart = (name) => cartItems.some((it) => it.name === name);

  // Plant data (all sections)
  const plantsArray = [
    {
  ...
  // === Back-to-top helper: scroll the overlay container or the window ===
  const scrollToTop = () => {
    const container =
      document.querySelector('.product-list-container.visible') ||
  ...
  return (
    <div>
      ...
      >
        ...
        {/* Right cluster: Plants (centered on mobile) + Cart (pinned on mobile via CSS) */}
        <div
        ...
        >
          ...
          <div className="cart-wrapper">
            <a href="#" onClick={handleCartClick} style={{ textDecoration: 'none' }}>
              ...
              >
                <img
                  src="https://cdn.pixabay.com/photo/2025/08/28/14/35/shopping-9802185_1280.png"
                  alt="Cart"
                  style={{
                    width: 30,
                    height: 30,
      ...
      {!showCart ? (
        <>
          ...
          {/* Floating Back-to-top button via portal (fixed bottom-right + smooth scroll) */}
          {createPortal(
            <button
